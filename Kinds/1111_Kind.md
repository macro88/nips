## Event Kind 1111 — Comment

### Overview

Event Kind **1111** defines **comment** in Nostr.

- **Specification**: [NIP-22](https://github.com/nostr-protocol/nips/blob/master/22.md)
    
- **Replaceability**: Addressable event. Only the latest event per combination of `kind`, `pubkey`, and `d` tag is valid.

---

### Schema Reference

|Key|Type|Max|Req|Format / Constraints|NIP Ref|Status|
|---|---|---|---|---|---|---|
|*To be documented*|*TBD*|*TBD*|*TBD*|*TBD*|[NIP-22](https://github.com/nostr-protocol/nips/blob/master/22.md)|*TBD*|

---

### Validation Rules

1. **Addressable** — addressable event rules apply.
    
2. **Follow NIP specifications** — adhere to the referenced NIP requirements.
    
3. **UTF-8** — all string fields must be valid UTF-8.

---

### Examples

**Basic Event**

```json
{
  "id": "...",
  "pubkey": "abc123...",
  "created_at": 1670000000,
  "kind": 1111,
  "tags": [],
  "content": "...",
  "sig": "..."
}
```

---

### Security & Pitfalls

- **Input validation**: validate all user-provided content.
    
- **Spam prevention**: implement appropriate rate limiting.
    
- **Privacy considerations**: be aware of metadata leakage.

---

### Client Checklist

-  Parse event content safely.
    
-  Verify UTF-8 encoding.
    
-  Follow replaceability rules.
    
-  Validate against NIP specifications.
    
-  Implement proper error handling.
